...
def generate_audio_files(sentences, output_dir):
    ...
    for idx, sentence in enumerate(sentences):
        ...
        try:
            # 检查 ref_free 变量，如果为 True，则将 prompt_text 设为空字符串
            ref_free = ref_free_var.get()
            prompt_text = "" if ref_free else str(
                prompt_text_entry.get()).strip()  # 如果 ref_free 为 True，prompt_text 为空，否则获取用户输入
            ...
            print(
                f"实际执行的参数：{sovits_model}, {gpt_model}, {prompt_text}"
                f"{ref_free}, {sentence}")
...
class VoiceGeneratorApp:
    def __init__(self, root, folder_path_var):
        ...
        self.merge_button = tk.Button(self.bottom_frame, text="合并语音", command=lambda: self.run_merge_audio_files([]))
    ...
    def generate_all_voices(self):
        ...
        # 启动一个新线程来合并音频文件
        merge_thread = threading.Thread(target=self.merge_audio_files, args=(failed_rows,))
        merge_thread.start()  # 启动合并线程
        self.voice_threads.append(merge_thread)  # 将合并音频的线程也保存到列表中

    def run_merge_audio_files(self, failed_rows):
        Thread = threading.Thread(target=self.merge_audio_files, args=(failed_rows,))
        Thread.start()
    ...
    def generate_character_voice(self, sovits_var, gpt_var, ref_wav_path_var, prompt_var, ref_language_var,
        ...
        try:
            ...
            if ref_free:
                ...
                prompt_text = prompt_var.get().strip()  # 获取提示文本

            ref_language = ref_language_var.get().strip()  # 获取参考音频语言
...
#程序打包指令1(先执行)：Pyinstaller -F -w -i sd.ico tts4.0.2.1.py
#程序打包指令2(后执行)：Pyinstaller -F -w -i sd.ico tts4.0.2.1.py --collect-data=gradio_client --collect-data=gradio
