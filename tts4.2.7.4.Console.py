...
class StartMenu:
    ...
    def open_settings_window(self):
        ...
        self.settings_window.title("设置 ACCESS_KEY 和 SECRET_KEY")
...
class VoiceGeneratorApp:
    ...
    def display_txt_files(self, main_folder):
        ...
        for idx, folder in enumerate(txt_files):
            ...
            combo_box_path.bind("<<ComboboxSelected>>",
                                lambda e, cbc=combo_box_charactor, cbp=combo_box_path_var: self.update_charactor_options(cbc, cbp))
            ...
            combo_box_charactor.bind("<<ComboboxSelected>>",
                                     lambda e, index=idx, char_var=charactor_var,
                                            sc=start_col, var=ref_wav_path_entry, result=prompt_var,
                                  ref_lang1=ref_language_var, ref_lang2=text_language_var, cbp=combo_box_path_var: self.on_charactor_selected(index,
                                                                                     char_var, sc, var.get(), result, ref_lang1, ref_lang2, cbp))
    ...
    def on_charactor_selected(self, idx, char_var, start_col, audio_file, prompt_var, ref_language_var, text_language_var, combo_box_path_var):
        """处理角色选择事件，更新配置文件和音频文件"""
    ...
    def generate_character_voice(self, sovits_var, gpt_var, ref_wav_path_var, prompt_var, ref_language_var,
                                 ...
                                 speed_var, ref_free_var, if_freeze_var, delimiter_var, idx, failed_rows, original_states, row_controls):
        if self.client is None:
            tk.messagebox.showerror("Error", "Not connected to Gradio server.")
            return
