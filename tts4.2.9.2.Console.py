...
api_lock_tengxun = threading.Lock()
api_lock_xunfei = threading.Lock()

if sys.platform == "win32":
    # 让程序自己处理DPI缩放，防止Windows自动缩放
...
class StartMenu:
    def __init__(self, root, text_preprocessor):
        ...
        self.project_name_combobox = ttk.Combobox(self.select_project_frame, width=26)
        self.project_name_combobox.grid(row=0, column=0, sticky='ew', padx=(1,0), pady=5)
        self.project_name_combobox.bind("<<ComboboxSelected>>", self.load_selected_project)  # 绑定选择事件

        # 删除项目按钮
        self.delete_project_button = ttk.Button(self.select_project_frame, text="删除", command=self.delete_project, width=5)
        ...
        self.settings_button = ttk.Button(left_frame, text="设置", command=self.open_settings_window)
        ...
        self.folder_path_entry.grid(row=1, column=0, sticky='ew', padx=(1,0), pady=5)
        self.select_folder_button = ttk.Button(right_frame, text="浏览...", command=self.select_folder, width=5)
        ...
        self.text_file_path_entry.grid(row=3, column=0, sticky='ew', padx=(1,0), pady=5)
        self.select_text_file_button = ttk.Button(right_frame, text="浏览...", command=self.select_text_file, width=5)
        ...
        # 新建 ChatGUI 按钮
        self.open_chat_button = ttk.Button(self.test_button_frame, text="测试大模型", command=self.open_chat_window,
                                          width=16)
        ...
        # 新建 GPT-SoVITS 按钮
        self.open_GPT_SoVITS_Button = ttk.Button(self.test_button_frame, text="测试GPT-SoVITS",
                                                command=self.toggle_example_window, width=16)
        self.open_GPT_SoVITS_Button.grid(row=0, column=2, padx=5, pady=(20, 5), sticky='we')

        # 创建项目按钮
        self.create_project_button = ttk.Button(path_frame, text="创建项目/进入项目", command=self.create_project)
    ...
    def load_selected_project(self, event):
        ...
        if selected_project != "新建项目":
            ...
            self.project_details_tree.delete(*self.project_details_tree.get_children())
    ...
    def open_settings_window(self):
        ...
        # 连接按钮
        connect_button = ttk.Button(gpt_sovits_tab, text="连接", command=self.connect_gradio, width=5)
        ...
        # 新建一个按钮来添加文本框
        self.add_textbox_button = ttk.Button(button_frame, text="添加预设路径", command=self.add_textbox)
        ...
        for service in services:
            ...
            if service == "KIMI":
                ...
                create_custom_config = ttk.Button(choose_custom_config_frame, text="保存",
                                                 command=self.create_custom_preset, width=5)
                create_custom_config.pack(side='left', padx=(0, 5))
                self.custom_config_combobox = custom_config

                delete_custom_config = ttk.Button(choose_custom_config_frame, text="删除",
                                                 command=lambda: self.delete_custom_preset(custom_config), width=5)
        ...
        if os.path.exists("config.json"):
            ...
            for content in character_folder_content:
                ...
                # 创建“浏览...”按钮并放置在文本框的右侧
                browse_button = ttk.Button(row_frame, text="浏览...", command=browse_folder, width=5)
                ...
                # 创建“删除”按钮并放置在浏览按钮的右侧
                delete_button = ttk.Button(row_frame, text="删除", command=delete_row, width=5)
        ...
        # 保存按钮
        save_button = ttk.Button(self.settings_window, text="保存", command=self.save_keys_and_close_window)
    ...
    def add_textbox(self):
        ...
        delete_button.grid(row=0, column=2, pady=5, padx=(0, 10), sticky="e")
    ...
    def create_scrollable_canvas(self):
        ...
        # 创建 Treeview
        self.project_details_tree = ttk.Treeview(frame, columns=("name", "info"), show="headings")
        self.project_details_tree.heading("name", text="项目")
        self.project_details_tree.heading("info", text="详细详细")
        self.project_details_tree.column("name", anchor="w")
        self.project_details_tree.column("info", anchor="w")

        # 创建垂直滚动条
        scrollbar = ttk.Scrollbar(frame, orient="vertical", command=self.project_details_tree.yview)
        self.project_details_tree.configure(yscrollcommand=scrollbar.set)

        # 使用 grid 布局可扩展
        self.project_details_tree.grid(row=0, column=0, sticky="nsew")
        scrollbar.grid(row=0, column=1, sticky="ns")

        # 设置 frame 中的 row 和 column 可扩展
        frame.rowconfigure(0, weight=1)
        frame.columnconfigure(0, weight=1)

        # 可选：绑定鼠标滚轮（Windows/Linux）
        self.project_details_tree.bind("<MouseWheel>",
                                       lambda e: self.project_details_tree.yview_scroll(-1 * int(e.delta / 120),
                                                                                        "units"))
        # macOS 支持
        self.project_details_tree.bind("<Button-4>", lambda e: self.project_details_tree.yview_scroll(-1, "units"))
        self.project_details_tree.bind("<Button-5>", lambda e: self.project_details_tree.yview_scroll(1, "units"))
    ...
    def update_project_details_in_canvas(self, project_name):
        """使用 Treeview 显示项目详细信息"""
        if project_name in self.projects:
            self.project_details_tree.delete(*self.project_details_tree.get_children())  # 清空之前的内容
            ...
            self.project_details_tree.insert("", "end", values=("项目名", project_name))
            self.project_details_tree.insert("", "end", values=("创建时间", created_time))
            self.project_details_tree.insert("", "end", values=("总字符数", char_count))
            self.project_details_tree.insert("", "end", values=("总行数", line_count))
            ...
            def insert_folder_file_stats(folder, section_name, add_spacing=False):
                ...
                if os.path.exists(folder):
                    ...
                    for file in files:
                        ...
                        self.project_details_tree.insert("", "end", values=(file, f"字符数: {chars}, 行数: {lines}"))
                else:
                    self.project_details_tree.insert("", "end", values=(section_name, "暂无"))
            ...
            if os.path.exists(chapters_tts_path):
...
class GPTSoVITSWindow:
    def __init__(self, root):
        ...
        tk.Entry(self.left_frame, textvariable=self.top_k_var).grid(row=2, column=1, padx=(5,30), pady=2, sticky="ew")

        # top_p
        tk.Label(self.left_frame, text="top_p:").grid(row=3, column=0, sticky="w", pady=2)
        self.top_p_var = tk.StringVar(value="1")
        tk.Entry(self.left_frame, textvariable=self.top_p_var).grid(row=3, column=1, padx=(5,30), pady=2, sticky="ew")

        # temperature
        tk.Label(self.left_frame, text="温度:").grid(row=4, column=0, sticky="w", pady=2)
        self.temperature_var = tk.StringVar(value="1")
        tk.Entry(self.left_frame, textvariable=self.temperature_var).grid(row=4, column=1, padx=(5,30), pady=2, sticky="ew")

        # 语速
        tk.Label(self.left_frame, text="语速:").grid(row=5, column=0, sticky="w", pady=2)
        self.speed_var = tk.StringVar(value="1")
        tk.Entry(self.left_frame, textvariable=self.speed_var).grid(row=5, column=1, padx=(5,30), pady=2, sticky="ew")

        # 无参考模式
        tk.Label(self.left_frame, text="无参考模式:").grid(row=6, column=0, sticky="w", pady=2)
        self.ref_free_var = tk.BooleanVar()
        tk.Checkbutton(self.left_frame, variable=self.ref_free_var).grid(row=6, column=1, padx=(0,30), pady=2, sticky="w")

        # 冻结设置
        tk.Label(self.left_frame, text="冻结设置:").grid(row=7, column=0, sticky="w", pady=2)
        self.if_freeze_var = tk.BooleanVar()
        tk.Checkbutton(self.left_frame, variable=self.if_freeze_var).grid(row=7, column=1, padx=(0,30), pady=2, sticky="w")
        ...
        # 导出按钮
        self.config4_frame = tk.Frame(self.right_frame)
        self.config4_frame.grid(row=0, column=1, sticky="nsew")
        tk.Label(self.config4_frame, text="").grid(row=0, column=0, sticky="w")
        self.output_button = ttk.Button(self.config4_frame, text="导出", command=self.export_character_config, width=5)
        self.output_button.grid(row=1, column=0, padx=5, pady=0, sticky="w")
        ...
        self.ref_wav_path.grid(row=3, column=0, padx=(1,0), pady=1, sticky="ew")
        ttk.Button(self.right_frame, text="浏览...", command=lambda: self.browse_file(self.ref_wav_path), width=5).grid(
            row=3, column=1,
            padx=5, pady=0,
            sticky="w")
        ...
        self.prompt_text_entry.grid(row=5, column=0, padx=(1,0), pady=(1,5), sticky="ew")
        ...
        # 刷新模型按钮
        ttk.Button(self.in_right_frame, text="刷新模型", command=self.run_update_model_choices, width=10).grid(row=0, column=0,
                                                                                                    pady=5,
                                                                                                    sticky="w")

        # 确认模型按钮
        ttk.Button(self.in_right_frame, text="确认模型", command=self.run_submit, width=10).grid(row=1, column=0, pady=5,
                                                                                      sticky="w")
        ...
        self.delimiter_entry.grid(row=12, column=0, padx=(1,0), pady=1, sticky="ew")
        ...
        self.output_dir_entry.grid(row=14, column=0, padx=(1,0), pady=1, sticky="ew")
        ttk.Button(self.right_frame, text="浏览...", command=lambda: self.browse_directory(self.output_dir_entry), width=5).grid(
            row=14,
            column=1,
            padx=5, pady=0,
            sticky="w")
        ...
        self.filename_prefix_entry.grid(row=16, column=0, padx=(1, 0), pady=1, sticky="ew")
        ...
        # 提交按钮
        ttk.Button(self.bottom_frame, text="生成语音", command=self.run_generate_audio_files).grid(row=0, column=3,
                                                                                                  pady=5,
                                                                                                  padx=10)
...
class StreamThread(QThread):
    ...
    @acquire_lock(api_lock_tengxun)
    def tengxun(self):
        # 从配置字典中获取腾讯 API 的认证信息和模型
        ali_config = self.config.get('腾讯', {})
        access_key = ali_config.get('ACCESS_KEY')
        api_key = ali_config.get('SECRET_KEY')
        selected_model = ali_config.get('MODEL')
...
class TextPreprocessor:
    def __init__(self, parent, start_menu):
        ...
        self.chapter_marked_contents = {}
        self.min_right_width = 320  # 右侧栏最小宽度
        self.min_left_width = 100  # 新增左侧最小宽度

        # 设置整体布局
        self.parent.grid_rowconfigure(0, weight=1)
        self.parent.grid_columnconfigure(0, weight=1)

        # 使用 PanedWindow 代替 grid 布局左右栏
        ...
        # 左侧 Frame 直接放文本框，无滚动条
        self.left_frame = tk.Frame(self.paned_window)

        # 右侧整体新建容器，包含滚动条和原来的 right_frame
        self.right_container_frame = tk.Frame(self.paned_window)

        # 原 right_frame 保持不变
        self.right_frame = tk.Frame(self.right_container_frame, width=300)
        self.right_frame.grid_propagate(False)
        self.right_frame.grid(row=0, column=1, sticky="nsew")

        # 右侧容器的列配置，列0是滚动条，列1是right_frame
        self.right_container_frame.grid_columnconfigure(1, weight=1)
        self.right_container_frame.grid_rowconfigure(0, weight=1)

        # 创建左侧文本框
        self.left_frame.grid_rowconfigure(0, weight=1)
        self.left_frame.grid_columnconfigure(0, weight=1)
        self.text_box = tk.Text(self.left_frame, wrap="word")
        self.text_box.grid(row=0, column=0, sticky="nsew", padx=(5,0), pady=5)

        # 右侧容器的滚动条
        self.text_scrollbar = tk.Scrollbar(self.right_container_frame, orient=tk.VERTICAL, command=self.text_box.yview)
        self.text_scrollbar.grid(row=0, column=0, sticky="ns", padx=(5, 0), pady=5)

        # 绑定滚动条与文本框
        self.text_box.configure(yscrollcommand=self.text_scrollbar.set)

        # 把 left_frame 和 right_container_frame 添加到最外层 paned_window
        self.paned_window.add(self.left_frame, weight=1)
        self.paned_window.add(self.right_container_frame)

        # bottom frame 放在 paned_window 下方
        self.bottom_frame = tk.Frame(self.parent)
        self.bottom_frame.grid(row=1, column=0, columnspan=2, pady=10, sticky="nsew")
        ...
        self.one_click_process = ttk.Button(self.bottom_frame, text="一键处理", command=self.run_process_all)
        self.one_click_process.pack(side=tk.LEFT, padx=10)

        # 下面原有的 right_frame 内部布局保持不变
        ...
        self.button_frame = tk.Frame(self.chapter_frame)
        self.button_frame.grid(row=0, column=1, padx=5, pady=5, sticky="e")

        self.delete_chapter_button = ttk.Button(self.button_frame, text="删除", command=self.delete_chapter, width=5)
        self.delete_chapter_button.pack(side="left", padx=2)

        self.save_chapter_button = ttk.Button(self.button_frame, text="保存", command=self.save_content, width=5)
        self.save_chapter_button.pack(side="left", padx=2)

        self.chapter_refresh_button = ttk.Button(self.button_frame, text="刷新", command=self.refresh_and_update, width=5)
        self.chapter_refresh_button.pack(side="left", padx=(2,0))

        # 首先创建一个外层 Frame，设置相同的边框样式
        self.canvas_outer_frame = tk.Frame(self.right_frame, bd=2, relief="groove")
        self.canvas_outer_frame.grid(row=3, column=0, sticky="nsew", padx=5, pady=5)

        # 然后将 Canvas 放入这个外层 Frame 中
        self.canvas = tk.Canvas(self.canvas_outer_frame, highlightthickness=0, bd=0)  # 移除 Canvas 自身的边框
        self.canvas.pack(fill="both", expand=True, padx=1, pady=1)  # 添加一点内边距以显示外层 Frame 的边框

        self.canvas_frame = tk.Frame(self.canvas, width=170)
        self.canvas_frame.bind("<Configure>", self.update_scroll_region)
        self.canvas.create_window((4, 5), window=self.canvas_frame, anchor="nw")
        ...
        # 复制按钮（默认隐藏）
        self.custom_copy_button = ttk.Button(self.canvas_frame, text="复制", command=self.copy_text, width=5)
        self.custom_copy_button.grid(row=10, column=0, padx=5, pady=5, sticky="w")
        self.custom_copy_button.grid_remove()

        # 更多模式文本框（默认隐藏）
        self.more_AI_entry = tk.Entry(self.canvas_frame, width=14)
        ...
        self.threshold_entry = tk.Entry(self.threshold_frame, width=12)
        ...
        self.retry_entry = tk.Entry(self.canvas_frame, width=14)
        ...
        self.text_limit_entry = tk.Entry(self.canvas_frame, width=14)
        ...
        # 创建处理按钮
        self.process_button = ttk.Button(self.canvas_frame, text="标注角色对话", command=self.run_process_dialogue)
        self.process_button.grid(row=16, column=0, padx=(5,0), pady=5, sticky="nw")
        ...
        self.process_frame.grid(row=16, column=1, padx=0, pady=5, sticky="e")
        ...
        # 检查最小宽度
        def enforce_min_widths(event=None):
            # 等待窗口尺寸稳定
            self.paned_window.update_idletasks()

            # paned_window宽度
            total_width = self.paned_window.winfo_width()
            if total_width <= 1:  # 防止在初始化时获取到无效宽度
                return

            # 左侧宽度 = paned_window第0个pane宽度
            left_width = self.paned_window.sashpos(0)
            # 右侧宽度 = total - left_width - sash宽度 (sash宽度一般是4，确保点在右侧)
            sash_width = 4
            right_width = total_width - left_width - sash_width

            # 检查左右宽度是否满足最小要求
            needs_adjust = False
            if right_width < self.min_right_width:
                new_left_width = total_width - self.min_right_width - sash_width
                if new_left_width < self.min_left_width:
                    # 如果调整右侧会导致左侧太小，优先保证左侧
                    new_left_width = self.min_left_width
                needs_adjust = True
            elif left_width < self.min_left_width:
                new_left_width = self.min_left_width
                needs_adjust = True

            if needs_adjust:
                # 确保新左侧宽度不会导致右侧太小
                new_right_width = total_width - new_left_width - sash_width
                if new_right_width < self.min_right_width:
                    # 如果还是不够，尝试调整窗口大小（如果可能）
                    new_left_width = total_width - self.min_right_width - sash_width
                    if new_left_width < self.min_left_width:
                        # 实在无法满足，优先保证左侧
                        new_left_width = self.min_left_width
                ...
                # 应用调整
                self.paned_window.sashpos(0, new_left_width)

        # 绑定事件
        self.paned_window.bind("<ButtonRelease-1>", enforce_min_widths)
        self.parent.bind("<Configure>", lambda e: enforce_min_widths())

        # 初始强制检查一次
        self.parent.after(100, enforce_min_widths)
    ...
    def process_all(self):
        ...
        for control in controls:
            ...
            # 处理其他控件
                ...
                original_states[control] = control.cget("state")  # 获取原始状态
                control.config(state="disabled")  # 禁用控件
            elif isinstance(control, ttk.Button):
...
class ChapterSplitterApp:
    def __init__(self, parent, text_processor):
        ...
        # 自定义模式文本框（默认隐藏）
        self.custom_entry = tk.Entry(self.parent, width=28)
        ...
        # 输入框
        self.entry = tk.Entry(self.parent, width=14)
        ...
        # 开始分割按钮
        self.split_button = ttk.Button(self.parent, text="自动分章", command=self.run_on_split_button_click, width=10)
...
class ChangeFormat:
    ...
    def create_controls(self):
        ...
        ttk.Button(self.control_frame, text="处理文本", command=self.process_text, width=10).grid(row=5, column=0, padx=5, pady=5,
                                                                                       sticky="w")
        # 检查文本内容并自动设置“将‘和’替换为‘「和」’”选项的状态
        # self.auto_set_replace_quotes()
...
class DocumentProcessorApp:
    ...
    def add_controls_to_canvas(self):
        ...
        self.symbol_entry = tk.Entry(self.parent, width=14)
...
class CharacterExtractor:
    def __init__(self, text_preprocessor, all_characters):
        ...
        self.replace_button = ttk.Button(
            ...
            text="替换角色",
            command=self.replace_characters, width=10
        )
...
class VoiceGeneratorApp:
    def __init__(self, root, folder_path_var, start_menu):
        ...
        # 新增 "导入配置" 按钮
        self.import_button = ttk.Button(self.path_frame, text="导入配置", command=self.import_all_configs, width=10)
        self.import_button.pack(side=tk.LEFT, padx=5)

        # 新增 "导出配置" 按钮
        self.export_button = ttk.Button(self.path_frame, text="导出配置", command=self.export_all_configs, width=10)
        ...
        # 浏览按钮
        self.browse_button = ttk.Button(self.path_frame, text="浏览...", command=self.browse_main_folder, width=5)
        self.browse_button.pack(side=tk.LEFT, padx=5)

        # 刷新列表按钮
        self.refresh_list_button = ttk.Button(self.path_frame, text="刷新列表", command=self.refresh_folder_list, width=10)
        self.refresh_list_button.pack(side=tk.LEFT, padx=5)

        self.refresh_model_button = ttk.Button(self.path_frame, text="刷新模型", command=self.refresh_model_list, width=10)
        ...
        self.merge_button = ttk.Button(self.bottom_frame, text="合并语音", command=self.run_merge_audio_files)
        ...
        self.generate_button = ttk.Button(self.bottom_frame, text="生成语音", command=self.start_generate_all_voices)
    ...
    def create_header(self, header_frame):
        headers = [
            ("角色名", 17), ("角色列表", 18), ("配置角色", 18), ("选择配置", 40), ("SoVITS模型", 21), ("GPT模型", 39),
            ("参考音频路径", 52), ("提示文本", 29), ("参考语言", 20), ("生成语言", 16), ("top_k", 5), ("top_p", 5),
            ("温度", 4),
            ("语速", 3), ("无参考模式", 8), ("冻结设置", 12), ("分句内切割方式", 16), ("分句符", 5)
        ]
...
class FolderAudioViewer:
    ...
    def create_header(self, header_frame):
        headers = [
            ("    头像", 12), ("角色名", 13), ("配置名称", 27), ("SoVITS模型", 21), ("GPT模型", 39),
            ("参考音频路径", 52), ("提示文本", 30),
...
# 程序打包指令1(先执行)：pyinstaller -F -w --icon=sd.ico --collect-data=gradio_client --collect-data=gradio .\.venv\Scripts\tts4.2.9.2.py